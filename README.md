...
- **Streaming**: assistant text is rendered edit-in-place with throttled edits (~1 per 1.5–2s); messages are chunked at platform limits by the router, not the adapters. `/render-thinking on` exposes thinking in a separate labelled stream with independent buffers, edits, chunking, resync, and finalization. The setting defaults off and persists per conversation. Tool-call rendering is an open experiment (see below).
...
- **Media**: inbound images are passed as image content parts; inbound files are saved into the session workspace with their path referenced in the prompt text. `/send <path>` reads one regular file contained by the bound workspace and dispatches a semantic outbound file to the selected adapter. Feishu sends supported images natively, MP4 as native media with a bundled neutral cover, and other files natively. Telegram sends JPEG/PNG as photos and everything else, including MP4, as documents. Telegram inbound deliberately supports one photo or one document with an optional caption, rejects albums and other media, and enforces the Bot API's 20 MB download limit.
- **State**: conversation→session mapping, permission mode, and thinking-display preference persist to `~/.kimi-bridge/state.json` with atomic writes and a versioned migration. Model, thinking effort, plan mode, and goal state remain authoritative in Kimi's public session surfaces; the bridge does not duplicate them in its state. Config is at `~/.kimi-bridge/config.toml`.
...
1. Grant tenant scopes `im:message.p2p_msg:readonly`, `im:message:readonly`, `im:message:send_as_bot`, `im:message:update`, and `im:resource` (or the narrower upload/download resource scopes available to the app).
...
- `/send <path>` for one outbound file. Relative paths resolve against the bound workspace; absolute paths are accepted only when their resolved target remains inside it. Directories, missing files, globs, multi-file arguments, and symlinks escaping the workspace are rejected.
- `/render-thinking [on|off]` to report or explicitly set separate thinking output. Enabling during a live turn backfills the current thinking snapshot; disabling freezes the visible thinking while the answer continues.

Read commands, including goal status, remain available while a turn is busy. `/title <text>`, `/tasks cancel <id>`, `/mode` changes, `/goal pause`, `/goal cancel`, `/send`, and `/render-thinking` are also allowed while busy; model, effort, plan, skill activation, compact, undo, goal creation, and goal resume reject instead of queueing. A mode change during a goal affects subsequent permission checks but does not resolve an approval or question that has already been issued. A non-command message submitted while a turn is already active is queued and steered into that turn at the next step boundary. Conversation bindings, permission modes, and thinking-display preferences are atomically persisted in `~/.kimi-bridge/state.json`; model, effort, plan, and goal state remain authoritative in Kimi's public session surfaces.
...
The Telegram adapter accepts plain text, one native photo, or one document with an optional caption. It rejects albums and other inbound media and enforces the hosted Bot API's 20 MB download ceiling. `/send` maps JPEG/PNG to `sendPhoto` and every other file to `sendDocument`; video is intentionally a document, not a video message. Approvals use inline buttons. Questions advance through one editable message; multi-select requires Done, Other requires an explicit reply to the generated prompt, and `/stop` cancels the active turn and interaction. Old buttons become stale after restart.

Start the bridge with `uv run kimi-bridge`. The Telegram adapter and outbound-file additions are implementation-complete and covered by fake Bot API tests, but real delivery, edit cadence, callback UX, thinking display, and file transfer remain unverified until credentials are available.
