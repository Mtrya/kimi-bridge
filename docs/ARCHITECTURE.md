...
Voice messages are text, not media or files: adapters decode them into semantic `InboundAudio` values (bytes plus an optional platform-native transcript), and the router resolves a transcript in layers — the configured `[voice.asr]` endpoint first when present, then the platform-native transcript. `src/kimi_bridge/speech.py` owns the platform-neutral Whisper-compatible HTTP transcription client; the Feishu adapter separately owns Feishu-native recognition because it is a platform capability. A found transcript enters the prompt prefixed with `[语音]` so the agent treats it as machine-transcribed speech and does not "correct" recognition errors; when no layer yields text, a system notice inside the prompt (never a user-visible platform reply) says the voice message could not be transcribed. Audio never uses the inbox-file path.
