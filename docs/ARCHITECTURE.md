...
Protocol-specific source projections live beside their platform adapters. `src/kimi_bridge/platforms/source_contract.py` provides only platform-neutral result and fetch contracts, while `scripts/check_platform_source_contracts.py` owns the bounded allowlisted client, report serialization, and rolling drift-issue synchronization. Pull-request tests use deterministic local fixtures; `.github/workflows/platform-source-contract-monitor.yml` performs the daily networked inspection on `main`.
