...
async def run() -> None:
    ...
    LOGGER.warning(
        "PERMISSION MODE IS AUTO: tool calls run without interactive approval. "
        "Use this bridge only with trusted users until interactive approvals exist."
    )
    ...
    async with supervisor:
        async with KimiServerClient(supervisor=supervisor) as client:
            ...
            try:
                await adapter.start(router.handle_inbound)
