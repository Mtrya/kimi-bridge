...
微信私聊能力已于 2026-08-08 基于腾讯 openclaw-weixin 的 `v2.4.6` 标签、使用一个白名单扫码账号完成真实环境验证。桥接进程一次只维持一个 Kimi 回复流：某个会话仍在生成回复时，其他账号发来的新模型请求会收到稍后重试的提示，不会中断前一个会话；不同账号的上下文隔离已通过自动化测试，真实环境验证只覆盖上述扫码账号。
