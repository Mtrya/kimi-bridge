...
kimi-bridge 负责控制 Kimi Code 的本地服务器，在重启后保持聊天与会话的绑定关系，按照各平台能力呈现模型回复，并把引导、文件、会话控制和平台支持的交互能力带进你的聊天客户端。
...
微信支持仅限扫码授权的机器人私聊，并已于 2026-08-08 基于腾讯 `v2.4.6` 标签完成真实验证。微信强制使用 `auto`，回复不可编辑，不提供审批、提问、独立思考流、群聊或主动推送。它支持接收图片、语音、文件和视频，以及发送图片、视频和文件；外发音频是普通可下载文件，不是原生语音消息。一个机器人授权只能由一个进程轮询。

## 功能特性

- 持久的 Kimi 会话管理：创建、列表、切换、重命名、查看、压缩和撤销。
- 原地编辑的答案流式输出、路由侧分块，以及可选的独立思考过程输出。
- 在平台支持时提供交互式审批与提问，并带超时处理和过期操作防护。
...
单元测试使用 Kimi、飞书、Telegram、QQ、微信、WebSocket、状态和进程边界的仿真实现；CI 检查不使用任何凭据或模型推理。
