...
Answers stream through editable messages where the platform supports edits and are split at the platform text limit. Bridge-generated command, status, validation, runtime error, and session warning replies are final messages. A post-submission turn failure is sent after any partial answer, and a retryable failure says that the request can be retried. Kimi session warnings are relayed when emitted. Background-task lifecycle events remain available through `/tasks` and do not produce unsolicited chat notices.
