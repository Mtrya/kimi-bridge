...
| --- | --- | --- | --- | --- | --- |
| Feishu | Official application registration; returns application credentials, not user OAuth | Print a Feishu/Lark registration URL and save confirmed managed credentials | Register a replacement without discarding the old credential before confirmation | Inspect local managed storage and redacted metadata; no network check | Remove only adapter-owned Feishu managed files; TOML fallback remains |
| QQ | Official bot credential bootstrap; returns bot app ID and locally decrypted AppSecret, not QQ user login or OAuth | Print a QQ bind URL and save the confirmed managed credential | Run a new bind flow without replacing the old credential until success | Inspect local managed storage and redacted metadata; no network check | Remove only adapter-owned QQ managed files; TOML fallback remains |
