...
kimi-bridge 负责监管 Kimi Code 的本地服务器，在重启后保持聊天与会话的绑定关系，以可编辑消息的形式流式输出回复，并把审批、提问、引导、文件、思考过程和会话控制带进你的聊天客户端。
...
- 繁忙回合中的提示引导、取消、权限模式、模型/推理强度/计划控制、目标、任务、技能，以及只读的 MCP 查看。
- 接收图片和文件，以及限制在工作区内的 `/send` 文件外发。
- 私聊白名单、仅监听回环地址的 Kimi 服务器监管，以及不启动任何组件、不泄露密钥的 doctor 诊断命令。
...
```
智能体会通过访谈了解你的需求，并端到端完成全部配置。

手动安装概要——需要先完成 [Kimi Code](https://moonshotai.github.io/kimi-code/en/guides/getting-started) 认证并安装 [uv](https://docs.astral.sh/uv/getting-started/installation/)：
...
在聊天中输入 `/help`，或阅读[命令参考](docs/COMMANDS.md)了解确切语法、会话繁忙时的行为以及各平台的媒体语义。发送 `/<命令> ?` 可查看详细的聊天内用法，包括 `/tasks show ?` 等子形式。

## 架构与安全

```text
飞书、QQ 或实验性的 Telegram
              │
              ▼
        语义化聊天路由器
              │
...
受监管的 Kimi 服务器使用随机生成的 bearer token 并仅绑定回环地址，聊天访问由适配器的白名单限制。kimi-bridge 为单一可信操作者设计，而非互不信任的多租户场景：获得许可的 Kimi 智能体可以以宿主机账户的权限读写文件、执行命令，因此请妥善保护宿主机和聊天凭据。组件边界与 Kimi Code 兼容性策略（`kimi-bridge compat`）见[架构文档](docs/ARCHITECTURE.md)。
...
单元测试使用伪造的 Kimi、飞书、Telegram、QQ、WebSocket、状态和进程边界；CI 检查不使用任何凭据或模型推理。
