...
**The managed server client, supported Feishu and QQ bridges, and experimental Telegram adapter are implemented, including interactive approvals/questions where available, prompt steering, inbound media, semantic outbound files, separately streamed optional thinking, compatibility diagnostics, and credential-free contract monitoring.** Feishu and QQ are live-validated; Telegram is fake-tested but not live-validated. QQ has no interactive approvals/questions or separate thinking stream (forced `auto` mode) but streams answers through a compact stable-frontier, append-monotonic `stream_messages` lifecycle with a withdrawal fallback for corrections to the rendered frontier. The core contracts are platform-neutral, and the runtime intentionally enables one selected adapter per process. Public behavior and architectural requirements live in `docs/ARCHITECTURE.md`, `docs/CONFIGURATION.md`, and `docs/COMMANDS.md`; changing those contracts requires explicit user sign-off and matching documentation in the same change.
