...
| --- | --- | --- | --- | --- | --- |
| Feishu | Official application registration; returns application credentials, not user OAuth | Print a Feishu/Lark registration URL and save confirmed managed credentials | Register a replacement without discarding the old credential before confirmation | Inspect local managed storage and redacted metadata; report a complete TOML pair when no managed credential exists; no network check | Remove only adapter-owned Feishu managed files; TOML fallback remains |
